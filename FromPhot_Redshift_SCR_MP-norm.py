...
def photometric_redshift(phototab):
    ...
    for i in range (nrow):
        ...
        if True:
            ...
            params.add('norm', value=(norm_func(fl[6])), vary=False)
            params.add('slope', value=0., min=-0.5, max=0.5)
            out = minimize(residual, params, args=(wsed, flsedA, flsedB, wo, stdo, bloo, bupo, lflo, erro, IGM_tau),
    ...
    if repeat2:  #check whether the fit was repeated with fixed z
        ...
        phototab_out.add_column(col_g)
        col_h = Column(name='slope2_fit', data=s2_fit)
        phototab_out.add_column(col_h)
        col_i = Column(name='chisq2', data=chi2)
        phototab_out.add_column(col_i)
    return phototab_out
...
from FromPhot_Redshift_SCR_MP import photometric_redshift, init_lock
import pandas as pd
...
if __name__ == '__main__':
    ...
    phototab_out=vstack(phototabs_out)
